...
const handleExport = () => {
  ...
  const exportData = filteredList.value.map(p => ({
    '編號': p.id, '姓名': p.name, '單位': p.company, '職稱': p.title,
    '電話': p.phone, 'Email': p.email, '身分': p.type, '報到狀態': p.status
  }));
  ...
  <div class="participants-view">
    <div class="page-header">
      <div class="title-group">
        ...
        <p class="subtitle">共計 {{ participants.length }} 位參與者 (篩選後：{{ filteredList.length }})</p>
      </div>
